...
ccc       double precision bgridmax(2),bgridmin(2),dd(0:3)
     ...
     &      critmax(2),causmin(2),causmax(2),critinmin(2),critinmax(2)
       ...
       double precision fampsum(5),famp_bndc(5)
     ...
     &         ica_not_inc(ncpts),ipt_caus(ncpts)
       ...
       integer iimage(5),iminside(5),included(5)
       integer izr(5),izphi(5)
         ...
         if(ncausin.gt.0) then
           do in = 1,ncausin
             if(ica_not_inc(in).gt.0) then
               ncausin2 = ncausin2 + 1
               if(icausin.eq.0) then
                 icausin = in
               elseif(igood_pt.eq.0) then
...
c          there should be no caustic points left now
c          ------------------------------------------
           ncausin3 = 0
           do in = 1,ncausin
             if(ica_not_inc(in).gt.0) then
           ...
           enddo
           if(ncausin3.gt.0) then
             write(6,*) ncausin3,
     &         ' internal caustic points missing from images'
           endif
         else
           nimcaus = 0
         endif
...
c        find the boundary
c        -----------------
         crosslocr=zbrentrad(starbndr,bra,brb,cosbphi,sinbphi,tol,
...
c        find the boundary
c        -----------------
         crosslocphi=zbrentrad(starbndphi,bphia,bphib,br,brdum,tol,
